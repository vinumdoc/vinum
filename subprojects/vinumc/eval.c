...
RESOLVE_FUNC_SIGNATURE(resolve_calls_call) {
	...
	if (symbol_info != NULL) {
		if (symbol_info->type == ENTRY_SYMBOL){
		...
		} else if (symbol_info->type == ENTRY_FUNCTION) {
...
void resolve_extern_functions (struct eval_ctx *ctx, size_t curr_scope_id, struct loaded_lib lib){
	struct scope *curr_scope = &VEC_AT(&ctx->scopes, curr_scope_id);
...
//TODO isso ta feio
struct loaded_lib *load_libs (struct eval_ctx *ctx, size_t curr_scope_id, struct libraries libs){
	struct loaded_lib *loaded_libs = malloc(sizeof(struct loaded_lib) * libs.counter);
...
void eval(struct eval_ctx *ctx, struct ast *ast, FILE *out, struct libraries libs) {
	...
	struct loaded_lib *loaded_libs = load_libs(ctx, 0, libs);
